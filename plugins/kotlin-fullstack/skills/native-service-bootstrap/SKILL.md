...
description: "Use when a NEW standalone Kotlin/Native service is being stood up (Ktor CIO + sqlx4k/SQLite + Helm) — from nothing to a deployed image: the Gradle skeleton and targets, ServerConfig through expect/actual, migrations and the SQLite pool size, DI, ordered shutdown and probes through kore, two allocators under a container limit (both carried by sborka.native-service, explained here), WAL truncation, a FROM scratch runtime image, the chart, CI with a ~/.konan cache. Mechanisms live in sborka's conventions and in kore; this skill says which to apply and what each was measured to buy. Triggers: 'bootstrap a new service', 'stand up a server from scratch', 'new Ktor service on Kotlin/Native', 'put the native binary in an image', 'write a chart for the service', 'set up CI for Kotlin/Native', 'the service is OOM-killed under its limit', 'the WAL keeps growing', 'image on scratch', 'adopt kore', 'заведи новый сервис', 'подними сервер с нуля', 'новый Ktor-сервис на Kotlin/Native', 'собери нативный бинарь в образ', 'напиши чарт для сервиса', 'настрой CI для Kotlin/Native', 'сервис убивает OOM под лимитом', 'растёт WAL', 'образ на scratch', 'внедри kore'. NOT for a new feature inside a service that already exists — that is ktor-server-feature."
---
